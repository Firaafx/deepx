...
let handDetectionConfidence = 0.3;
let handTrackingConfidence = 0.3;
...
function applyRuntimeSettings(settings) {
    ...
    handDetectionConfidence = Math.max(0.2, Math.min(0.99, handDetectionConfidence));
    handTrackingConfidence = Math.max(0.2, Math.min(0.99, handTrackingConfidence));
    ...
    if (hands) {
        hands.setOptions({
            modelComplexity: 0,
            maxNumHands: 1,
            minDetectionConfidence: Math.max(0.2, Math.min(0.99, handDetectionConfidence)),
            minTrackingConfidence: Math.max(0.2, Math.min(0.99, handTrackingConfidence)),
        });
    }
}
        ...
        onFrame: async () => {
            ...
            if (shouldProbeHand) {
                try {
                    ...
                    console.warn('Hands pipeline dropped one frame.', err);
                    hasHand = false;
                    handLm = null;
                    isPinching = false;
                    handStableFrames = 0;
